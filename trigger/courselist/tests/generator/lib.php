...
/**
 * lifecycletrigger_courselist generator tests
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_lifecycle_trigger_courselist_generator extends testing_module_generator {
    ...
    public static function create_trigger_with_workflow($data) {
        ...
        // Create trigger.
        $record = new stdClass();
        $record->subpluginname = 'courselist';
        $record->instancename = 'courselist';
        $record->workflowid = $workflow->id;
        $trigger = trigger_subplugin::from_record($record);
        trigger_manager::insert_or_update($trigger);
        // Set delay setting.
        $settings = new stdClass();
        $settings->courses = $data['courses'];
        $settings->operation = $data['operation'];
